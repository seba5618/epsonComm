...
public class ASCII85Decode {
        ...
        for (i = 0; i < 5; i++) {
            ...
            if (five[i] == '~') {
                if (nextChar() == '>') {
                    ...
                    throw new IOException("Bad character in ASCII85Decode: not ~>");
                }
                ...
                if (i == 0) {
                    ...
                    throw new IOException("Inappropriate 'z' in ASCII85Decode");
                }
            } else {
                throw new IOException("Bad character in ASCII85Decode: " + five[i] + " (" + (char) five[i] + ")");
            }
        }
     ...
     */
    private ByteBuffer decode() throws IOException {
    ...
    public static byte[] decode(String buf)
            throws IOException {
        ByteBuffer byteBuffer = ByteBuffer.wrap(buf.getBytes());
        ASCII85Decode me = new ASCII85Decode(byteBuffer);
        ByteBuffer decode = me.decode();
        byte[] arr = new byte[decode.remaining()];
        decode.get(arr);
        return arr;
    }
}